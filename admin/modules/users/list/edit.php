...
if (@$_POST['hidden']) {
    ...
    if (!@$erorr) {
        ...
        $sql = "update i_" . $ob->pr($_GET['module']) . " set id_group='" . @$_POST['id_group'] . "', ".($_POST['login']!=''?"login='" . sha1(@$_POST['login']) . "',":"")."  ".($_POST['password']!=''?"password='" . sha1(@$_POST['password']) . "',":"")." active='" . @$_POST['active'] . "', name='" . @$_POST['name'] . "', last_name='" . @$_POST['last_name'] . "', email='" . @$_POST['email'] . "', timestamp_x='".date('Y-m-d H:i:s')."' where id='".intval($_GET["id"])."'";
...
if (isset($_GET["id"])){
    $sql = "select * from i_user where id='".intval($_GET["id"])."'";
  ...
  <header id="topbar" class="alt" style="padding-bottom: 0px;">
    <div class="topbar-left pull-left">
      <ol class="breadcrumb">
        ...
        <li class="crumb-active"> Редактирование пользователя</li>
    </ol>
</div>
</header>
<section id="content" class="table-layout">
                  ...
                  <div class="section row mb10">
                   ...
                   <div class="col-md-9">
                    <select name="id_group" id="id_group" class="form-control">
                      ...
                      while($res1=$select->fetch_array())
                      {
                        echo '<option value="'.$res1['id'].'" '.($res1["id"]==@$resU["id_group"]?'selected':'').'>'.$res1['name'].'</option>';
                    }
                    ?>
                </select>
            </div>
  ...
  <div class="section row mb10">
   ...
   <div class="col-md-9">
    <input name="last_name" type="text" id="last_name" size="35" class="form-control" value="<?=@$resU["last_name"]?>" />
</div>
</div>
<div class="section row mb10">
  <label class="field-label col-md-3 text-right">
 ...
 <div class="col-md-9">
  <input name="email" type="text" id="email" size="35" class="form-control" value="<?=@$resU["email"]?>" />
</div>
</div>
<div class="section row mb10">
    <label class="field-label col-md-3 text-right">
