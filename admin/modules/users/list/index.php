...
if(@$_POST['actions']){
	if(count(@$_POST['select_ch'])>0){
		...
		if($_POST['actions']=="action"){$query="update i_user set active='1' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="deaction"){$query="update i_user set active='0' where id='".$ob->pr($v)."'";}
		if($_POST['actions']=="delete"){$ob->del_r("i_user",array("id"=>$v));}
		@$mysql->query(@$query);
		}
...
{
	$ob->update("i_user",array("active"=>$_GET['active']),$_GET['id'],"");
}

if(@$_GET['delete']=="true" AND $_GET['id'])
{
$ob->del_r("i_user",array("id"=>$_GET['id']));
}

//началась выборка
$field = array("id"=>"ID","active"=>"Активность","id_group"=>"Группа","name"=>"Имя","last_name"=>"Фамилия","email"=>"E-mail");
$where='';
    ...
    <section id="content" class="table-layout">
                ...
                <div class="panel-body">
                    ...
                    <table width="100%" class="table table-responsive table-hover table-bordered" style="color: #000; ">
                        ...
                        while ($res = $select->fetch_array()) {
                            ...
                            $sql = "select * from i_user_group where id='".$res["id_group"]."'";
      ...
      //-->
                            ...
                            <td class="id_block_style" align="center">' . $res['id'] . '</td>
                             ...
                             <td class="id_block_style" align="center">' . $res1['name'] . '</td>
                              <td class="id_block_style" align="center">' . $res['name'] . '</td>
                               <td class="id_block_style" align="center">' . $res['last_name'] . '</td>
                               <td class="id_block_style" align="center">' . $res['email'] . '</td>
