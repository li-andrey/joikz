...
if ($_POST['login']!=''){
        $sql = "select * from i_" . $ob->pr($_GET['module']) . " where login='".(@$_POST['login'])."' and id!='".intval($_GET["id"])."' ";
        $res = $mysql->get($sql, 1);
        if (@$res["id"]>0){
...
if ($_POST['mail']!=''){
        $sql = "select * from i_" . $ob->pr($_GET['module']) . " where mail='".(@$_POST['mail'])."' and id!='".intval($_GET["id"])."' ";
        $res = $mysql->get($sql, 1);
        if (@$res["id"]>0){
...
{   
    ...
    $sql = "update i_" . $ob->pr($_GET['module']) . " set active='" . @$_POST['active'] . "' ".($_POST['login']!=''?", login='" . (@$_POST['login']) . "'":"")."  ".($_POST['mail']!=''?", mail='" . (@$_POST['mail']) . "'":"")."   ".($_POST['password']!=''?", password='" . sha1(@$_POST['password']) . "'":"")."  where id='".intval($_GET["id"])."'";
                        ...
                        <div class="panel  top mb35">
                            <div class="panel-body bg-white">
                              <div class="admin-form">
                                ...
                                if (isset($_GET["id"])){
                                    $sql = "select * from i_shop_users where id='".intval($_GET["id"])."'";
                               ...
                               <div class="col-md-9">
                                  <input name="mail" type="text" id="email" size="35" class="form-control" value="<?=@$resU["mail"]?>" />
                              </div>
                          </div>
                          <div class="section row mb10">
                            <label class="field-label col-md-3 text-right">
                         ...
                         <div class="col-md-9">
                            <input name="login" type="text" id="login" size="35" class="form-control" required=""  value="<?=@$resU["login"]?>" />
                        </div>
                    </div>
                    <div class="section row mb10">
                      <label class="field-label col-md-3 text-right">
