...
if (@$_GET['id'] and isset($_GET['active'])) {
  $ob->update("i_$nameElement", array("active" => $_GET['active']), $_GET['id'], "");
}

if (@$_GET['copyr'] == "true" and $_GET['id']) {
    $return = duplicate('i_'.$nameElement, 'id', $_GET["id"]);
    $ob->go("elements.php?module=".$nameElement."&sub_module=".$nameRazdel."&id_section=" . $_GET["id_section"]);
}
...
if (!isset($_GET["order"])) {
  if (@$_SESSION[$nameElement]["order"] == '') {
    $_SESSION[$nameElement]["order"] = "id asc";
  }
} else {
  $_SESSION[$nameElement]["order"] = $ob->pr($_GET['order']);
}
...
if (!isset($_GET["start"])) {
  if (@$_SESSION[$nameElement]["start"] == '') {
    $_SESSION[$nameElement]["start"] = 0;
  }
} else {
  $_SESSION[$nameElement]["start"] = $ob->pr($_GET['start']);
}
...
if (!isset($_GET["number"])) {
  if (@$_SESSION[$nameElement]["number"] == '') {
    $_SESSION[$nameElement]["number"] = 20;
  }
} else {
  $_SESSION[$nameElement]["number"] = $ob->pr($_GET['number']);
}
if ($_SESSION[$nameElement]["number"] == '' || $_SESSION[$nameElement]["number"] == 0) {
  $_SESSION[$nameElement]["number"] = 20;
}

$sql = "select * from i_$nameElement where (version='all' OR version='" . $_SESSION[$nameElement]['version'] . "') ";
$sql.= "" . $where . " ORDER BY " . $_SESSION[$nameElement]["order"] . ",'puttime' DESC LIMIT " . $_SESSION[$nameElement]["start"] . "," . $_SESSION[$nameElement]["number"] . "";
...
if ($select->num_rows == 0) {
  $_SESSION[$nameElement]["start"] = 0;
  $sql = "select * from i_$nameElement where (version='all' OR version='" . $_SESSION[$nameElement]['version'] . "') ";
  $sql.= "" . $where . " ORDER BY " . $_SESSION[$nameElement]["order"] . ",'puttime' DESC ";
  $sql.= "LIMIT " . $_SESSION[$nameElement]["start"] . "," . $_SESSION[$nameElement]["number"] . "";
  $select = $mysql->query($sql);
}

$select_all = $mysql->query("select * from  i_$nameElement where (version='all' OR version='" . $_SESSION[$nameElement]['version'] . "') " . $where . "");
?>
  ...
  <section id="content" class="table-layout">
    <div class="tray tray-center" style="padding-top: 0px;">
            ...
            <div class="panel">
              <div class="panel-heading">
                <div class="widget-menu pull-left">
                  ...
                  <a href="add_elements.php<?=$ob->gets_go($_GET, "module", $nameElement)?>" class="btn btn-system btn-sm">
                    <span class="fa fa-plus"></span> Добавить элемент
                  </a>

                </div>
                <div class="widget-menu pull-right">
                  <a href="option.php?module=<?=$nameElement?>&amp;id_section=<?=@$_GET['id_section']?>" target="_self" class="btn btn-system  btn-sm">
                 ...
                 <a href="import.php<?=$ob->gets_go($_GET, "", "")?>" target="_self" class="btn btn-system btn-sm" style="margin-right: 20px;">
                 ...
                 <select name="jumpMenu" id="jumpMenu" class="form-control" style="display: inline-block; width: auto;" onchange="MM_jumpMenu('parent',this,0)">
                  ...
                  foreach ($elements as $k => $v) {
                    if ($v == $_SESSION[$nameElement]["number"]) {$sel = 'selected';} else { $sel = '';}
            ...
            <div class="panel-body">
                ...
                <table width="100%" class="table table-responsive table-hover table-bordered" style="color: #000; ">
                  <thead>
                    <tr class="dark">
                      ...
                      foreach ($field as $k => $v) {
                        ...
                        onclick="document.location.href=\'elements.php' . $ob->gets_go($_GET, "order", $k) . '\'"
                  ...
                  while ($res = $select->fetch_array()) {
                    ...
                    ondblclick="javascript:document.location.href=\'edit_elements.php' . $ob->gets_go($_GET, "id", $res['id']) . '&module='.$nameElement.'\'" >
                    ...
                    <li ' . $li_style . '><a href="edit_elements.php' . $ob->gets_go($_GET, "id", $res['id']) . '&module='.$nameRazdel.'">Редактировать</a></li>
      ...
      //-->
                    ...
                    $field2 = $ob->search_option($nameElement, $_GET['sub_module'], @$_GET['id_section'], array("select_fields" => 1));
                  ...
                  foreach ($field as $k => $v) {
                    ...
                    if ($k == 'url') {
                      ...
                      echo '<td class="table_value"><a href="/' .($_SESSION[$nameElement]["version"]!='ru'?$_SESSION[$nameElement]["version"].'/':'')  . '' . $res['url'] . '/">/' .($_SESSION[$nameElement]["version"]!='ru'?$_SESSION[$nameElement]["version"].'/':'')  . '' . $res['url'] . '/</a>&nbsp;</td>';
              ...
              <div  style="margin-top: 10px; text-align: center;">
                <div class="pagination" style="display: inline-block; padding: 0px 0px; margin: 0px 0px 10px 0;">
                  ...
                  $por   = ceil($count / $_SESSION[$nameElement]["number"]);
                  $link  = 0;
                  $right = 0;

                  for ($i = 1; $i <= $por; $i++) {
                    if ($_SESSION[$nameElement]["start"] <= $link) {
                      $right++;
                      if (($right == 1) and $_SESSION[$nameElement]["start"] > 0) {
                        if (($i - 5) > 0) {$minus = $i - 5;} else { $minus = 0;}
                        $link_main = $link - ($_SESSION[$nameElement]["number"] * 5);
                      ...
                      if ($right <= 5) {
                        if ($_SESSION[$nameElement]["start"] == $link) {$style = 'style="font-weight:bold"';} else { $style = '';}
                          echo '<a href="' . $ob->gets_go($_GET, "start", $link) . '" class="small_text" ' . $style . '>' . $i . '</a>&nbsp;';
                        }
                        if (($right == 5) and (($link + $_SESSION[$nameElement]["number"]) < $count)) {
                          echo '<a href="' . $ob->gets_go($_GET, "start", ($link + $_SESSION[$nameElement]["number"])) . '" class="small_text" >[' . ($i + 1) . ".." . ($i + 5) . ']</a>';
                        }
                      }
                      $link = $_SESSION[$nameElement]["number"] + $link;
                    }
                    ?>
                  </div>
                </div>
...
if (@$_GET['excel'] == "true") {
  ...
  $select = $mysql->query("select * from i_$nameElement where (version='all' OR version='" . $_SESSION[$nameElement]['version'] . "') " . $where . " ORDER BY " . $_SESSION[$nameElement]["order"] . ",'puttime' DESC ");
 ...
 $field1 = $field;
    ...
    while ($res = $select->fetch_array()) {
        $section  = $ob->select("i_".$nameRazdel, array("id_section" => $res['id']), "");
        $elements = $ob->select("i_".$nameElement, array("id_section" => $res['id']), "");

        $str .= '<tr ><td class="table_value">' . $res['id'] . '&nbsp;</td>';
        $field = $ob->search_option($nameElement, $_GET['sub_module'], @$_GET['id_section'], array("select_fields" => 1));
        ...
        foreach ($field1 as $k => $v) {
            if ($k == 'url') {
                $str .= '<td class="table_value"><a href="/' .($_SESSION[$nameElement]["version"]!='ru'?$_SESSION[$nameElement]["version"].'/':'')  . '' . $res['url'] . '/">/' .($_SESSION[$nameElement]["version"]!='ru'?$_SESSION[$nameElement]["version"].'/':'')  . '' . $res['url'] . '/</a>&nbsp;</td>';
            }
        }
