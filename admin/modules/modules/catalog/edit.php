...
if(@$_POST['hidden']){
    ...
    if (isset($_POST["cat"])) {
        ...
        foreach ($_POST["cat"] as $key => $r) {
            if ($r!=''){
                $sql = "insert into i_shop_cat set id_shop=".intval($_GET["id"]).", id_cat='".$r."'";
                A::$db->query($sql);
            }
        }
        $cat = $_POST["cat"][0];
    ...
    if (isset($_POST["ids"])){
        $mysql->query("update i_cat_elements set id_section='".intval($cat)."' where id=".intval($_GET["id"])."");
        //$_GET["id_section"] = $cat;
    }
...
if ($_POST['hidden']=='save'){
    $page = "index.php".$ob->gets_go($_GET,"","");
}else{
    $page = "";
}
...
$s = $mysql->query("select * from i_$nameRazdel where id=".intval($_GET["id"])."");
$r = $s->fetch_array();
...
{	
	...
	alias($_GET['id'], $nameRazdel, $translit, 1);
    ...
    <header id="topbar" class="alt" style="padding-bottom: 0px;">
        <div class="topbar-left pull-left">
            <ol class="breadcrumb">
                ...
                ?>
                <li class="crumb-active"> Редактирование категории</li>
            </ol>
        </div>
    </header>
                                        ...
                                        if ($option_res['type_field'] == "i10") {
                                            ...
                                            if ($option_res['name_en'] == 'link') {
                                                ...
                                                if ($option_res['name_en']=='cat'){
                                                    $sql = "select * from i_cat where id='".intval($_GET["id"])."'";
                                                    $res1 = A::$db->get($sql, 1);
                                                    $sql = "select * from i_shop_cat where id_shop='".intval($_GET["id"])."'";
                                                    $res = A::$db->get($sql);
                                                    $cat = array();
                                                    ...
                                                    foreach ($res as $key => $v) {
                                                        ...
                                                        foreach ($res1 as $key1 => $v1) {
                                                            echo '<p style="font-size:14px; padding-left:20px; font-weight:bold; text-transform:uppercase; " ><label><input type="checkbox" name="cat[]" value="'.$v1["id"].'" '.(in_array($v1["id"], $cat)?'checked':'').'> '.$v1["name"].'</label></p>';
                    ...
                    <div id="tab2_2" class="tab-pane ">
                        <div class="panel  top mb35">
                            <div class="panel-body bg-white">
                              <div class="admin-form">
                                <?
                                $sql = "select url from i_$nameRazdel where id='".intval($_GET["id"])."'";
                                $res = A::$db->get($sql, 1);
                                echo '<div class="section row mb10">
                                <label class="field-label col-md-3 text-right">
                                URL:
