...
if(@$_POST['hidden']){
    ...
    if (isset($_POST["ids"])){
        $mysql->query("update i_cat_elements set id_section='".intval($_POST["ids"])."' where id=".intval($_GET["id"])."");
        $_GET["id_section"] = $_POST["ids"];
    }
                            ...
                            <div class="panel  top mb35">
                                <div class="panel-body bg-white">
                                  <div class="admin-form">
                                    <?
                                    $sql = "select id_section from i_cat_elements where id='".intval($_GET["id"])."'";
                                    $resOne = $mysql->get($sql, 1);
                                    echo '<div class="section row mb10">
                                    <label class="field-label col-md-3 text-right">Категория:</label>
                                    <div class="col-md-9">
                                    <select name="ids" class="form-control" id="ids">
                                    ';
                                    $sql = "select * from i_cat where id_section=0 and (version='" . $_SESSION["version"] . "' or version='all') order by id asc";
                                    $ress = $mysql->get($sql);
                                    foreach ($ress as $key => $r) {
                                        echo '<option value="'.$r["id"].'" '.($resOne["id_section"]==$r["id"]?'selected':'').'>'.$r["name"].'</option>';
                                        $sql1 = "select * from i_cat where id_section=".$r["id"]." and (version='" . $_SESSION["version"] . "' or version='all') order by name asc";
                                        $ress1 = $mysql->get($sql1);
                                        foreach ($ress1 as $key1 => $r1) {
                                            echo '<option value="'.$r1["id"].'" '.($resOne["id_section"]==$r1["id"]?'selected':'').'>---'.$r1["name"].'</option>';
                                            $sql2 = "select * from i_cat where id_section=".$r1["id"]." and (version='" . $_SESSION["version"] . "' or version='all') order by name asc";
                                            $ress2 = $mysql->get($sql2);
                                            foreach ($ress2 as $key2 => $r2) {
                                                echo '<option value="'.$r2["id"].'" '.($resOne["id_section"]==$r2["id"]?'selected':'').'>------'.$r2["name"].'</option>';
                                                $sql3 = "select * from i_cat where id_section=".$r2["id"]." and (version='" . $_SESSION["version"] . "' or version='all') order by name asc";
                                                $ress3 = $mysql->get($sql3);
                                                foreach ($ress3 as $key3 => $r3) {
                                                    echo '<option value="'.$r3["id"].'" '.($resOne["id_section"]==$r3["id"]?'selected':'').'>---------'.$r3["name"].'</option>';
                                                    $sql4 = "select * from i_cat where id_section=".$r3["id"]." and (version='" . $_SESSION["version"] . "' or version='all') order by name asc";
                                                    $ress4 = $mysql->get($sql4);
                                                    foreach ($ress4 as $key4 => $r4) {
                                                        echo '<option value="'.$r4["id"].'" '.($resOne["id_section"]==$r4["id"]?'selected':'').'>------------'.$r4["name"].'</option>';
                                                    }
                                                }
                                            }
                                        }
