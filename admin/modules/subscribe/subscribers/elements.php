...
$nameRazdel = 'subscriber';
$nameElement = 'subscriber_elements';
$nameModule = 'subscribers';
$i = 1;
                ...
                if ($_POST['subscribe'] != "not") {
                    ...
                    try {
                        ...
                        foreach ($images[1] as $k => $v) {
                          if (strstr($v, 'images')) {
                            $mail->AddEmbeddedImage($_SERVER['DOCUMENT_ROOT'] . "/upload/userfiles/images/" . basename($v), 'mail-img-' . $k, basename($v), 'base64', 'image/jpeg');
                        } else {
                    ...
                    foreach ($files as $k => $v) {
                        if ($v != "") {
                            $mail->AddAttachment($_SERVER['DOCUMENT_ROOT'] . "/upload/subscribe/" . $v);
                        }
                    }
      ...
      <div class="tray-inner">
        <div class="row flex-column-reverse-before-md">
          <div class="col-sm-12">
            ...
            |  <select name="subscribe" id="subscribe" style="width:130px; display: inline-block;" class="form-control">
                <option value="not">выбор рассылки</option>
                <?
                $sub = $ob->select("i_subscribes", array("act" => 1, "version" => $_SESSION['version']), "id DESC");
                while ($sub_res = $sub->fetch_array()) {
                    echo '<option value="' . $sub_res['id'] . '">' . $sub_res['name'] . '</option>';
                }
                ?>
            </select> |  <select name="actions" id="actions" style="width:130px; display: inline-block;" class="form-control">
              <?
              $actions = array("" => "выбор действия", "send" => "Разослать", "active" => "Показывать", "notactive" => "Не показывать", "delete" => "Удалить");
...
if (@$_GET['excel'] == "true") {
  ...
  $select = $mysql->query("select * from i_$nameElement where (version='all' OR version='" . $_SESSION['version'] . "') " . $where . " ORDER BY " . $_SESSION["order"] . ",'puttime' DESC ");
