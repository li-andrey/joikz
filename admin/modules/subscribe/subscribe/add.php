...
if (@$_POST['hidden']) {
	$mysql->query("INSERT INTO i_subscribes(`version`,`act`, `date`, `mail_from`, `name`, `theme`, `message`, `files`) VALUES('" . $_SESSION['version'] . "', '" . (@$_POST['act'] ? 1 : 0) . "', '" . date("Y.m.d") . "', '" . $_POST['mail_from'] . "', '" . $_POST['name'] . "', '" . $_POST['theme'] . "', '" . $_POST['full_text'] . "', '" . $_POST['upl_files'] . "')");
  if ($_POST['hidden'] == 'save') {$page = "index.php";} else { $page = "";}
  $mysql->query("insert into i_logs values(0, CURRENT_TIMESTAMP,'" . $_SESSION['user_id'] . "','add','subscribes', '".@$_POST["name"]."', '', '')");
  $ob->go($page);
}
?>
  ...
  <header id="topbar" class="alt" style="padding-bottom: 0px;">
    <div class="topbar-left pull-left">
      <ol class="breadcrumb">
        ...
        ?>
        <li class="crumb-active"> Добавление рассылки</li>
      </ol>
    </div>
  </header>
                    ...
                    <div class="panel-body bg-white">
                      <div class="admin-form">
                        <div class="section row mb10">
                          ...
                          <div class="col-md-9">
                            <label style="line-height: 42px;"><input name="act" type="checkbox" id="act" value="1" /></label>
                          </div>
                        </div>
                        <div class="section row mb10">
                          <label class="field-label col-md-3 text-right">Отсылать с e-mail:</label>
                          <div class="col-md-9">
                            <input name="mail_from" class="form-control" type="text" id="mail_from" value="<?=("Administration@" . $_SERVER['HTTP_HOST'])?>" size="35" />
                          </div>
                        </div>
                        <div class="section row mb10">
                          <label class="field-label col-md-3 text-right">Название рассылки:</label>
                          <div class="col-md-9">
                            <input name="name" class="form-control" type="text" id="name" size="35" />
                          </div>
                        </div>
                        <div class="section row mb10">
                          <label class="field-label col-md-3 text-right">Тема рассылки:</label>
                          <div class="col-md-9">
                            <input name="theme" class="form-control" type="text" id="theme" size="35" />
                          </div>
                        </div>
                        <div class="section row mb10">
                          <label class="field-label col-md-3 text-right">Файлы:</label>
                          <div class="col-md-9">
                            <div id="uploaders-container"></div><div id="uploaded-files-container"></div>
                          </div>
                        </div>
                        <div class="section row mb10">
                          <label class="field-label col-md-3 text-right">Текст:</label>
                          <div class="col-md-9">
                            <textarea id="full_text" name="full_text" value=""></textarea>
...
<script type="text/javascript">
  ...
  function replainFiles() {
    if (uploaded_files) {
      ...
      var str = '<table style="margin-top:30px;" width="100%" class="table table-bordered"><tbody><tr class="small_text" align="center" style="color:#572a00; background-color:#ecebcf;"><td style="border:1px solid #545644;" width="50%">Файл</td><td style="border:1px solid #545644;" width="25%">Размер</td><td style="border:1px solid #545644;" width="25%">Действие</td></tr>';
      var check = 0;
      for (var i in uploaded_files) {
        if (uploaded_files[i].file) check++;
        str += '<tr class="small_text" align="center"><td><a href="/upload/subscribe/' + uploaded_files[i].file + '" target="_blank">' + uploaded_files[i].file + '</a></td><td>' + (uploaded_files[i].size / 1024).toFixed(2) + ' KB </td><td><a href="javascript:void(0);" onClick="if(confirm(\'Вы уверены что хотите удалить этот файл?\')) deleteUploadedFile(' + (i) + ');">Удалить</a></td></tr>';
      }
      str += '</tbody></table>';
