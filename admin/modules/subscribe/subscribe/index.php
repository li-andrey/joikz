...
if (@$_POST['actions']) {
    if (count(@$_POST['select_ch']) > 0) {
        foreach ($_POST['select_ch'] as $k => $v) {
            if ($_POST['actions'] == "action") {$query = "update i_$nameRazdel set act='1' where id='" . $ob->pr($v) . "'";}
            if ($_POST['actions'] == "deaction") {$query = "update i_$nameRazdel set act='0' where id='" . $ob->pr($v) . "'";}
            if ($_POST['actions'] == "delete") {$ob->del_r("i_$nameRazdel", array("id" => $v));}
            $mysql->query(@$query);
        }
...
if (@$_GET['id'] and isset($_GET['active'])) {
    $ob->update("i_".$nameRazdel, array("act" => $_GET['active']), $_GET['id'], "");
}

if (@$_GET['delete'] == "true" and $_GET['id']) {
    $ob->del_r("i_".$nameRazdel, array("id" => $_GET['id']));
}

//началась выборка
$field = array("id" => "ID","act" => "Активность", "date" => "Дата", "name" => "Название", "theme" => "Тема");
$where = '';
