...
if(@$_POST['actions'])
{
    if(count(@$_POST['select_ch'])>0)
    {
        ...
        {
            if($_POST['actions']=="action"){$query="update i_lang set active='1' where id='".$ob->pr($v)."'";}
            if($_POST['actions']=="deaction"){$query="update i_lang set active='0' where id='".$ob->pr($v)."'";}
            if($_POST['actions']=="delete"){$ob->del_r("i_lang",array("id"=>$v));}
            @$mysql->query(@$query);
        }
...
{
    $ob->update("i_lang",array("active"=>$_GET['active']),$_GET['id'],"");
}

if(@$_GET['delete']=="true" AND $_GET['id'])
{
    $ob->del_r("i_lang",array("id"=>$_GET['id']));
}

//началась выборка
$field=array("active"=>"Активность","name"=>"Наименование","name_reduction"=>"Сокращение");
$where='';
...
<form id="form_index" name="form_index" method="post" action="">
    ...
    <section id="content" class="table-layout">
                            ...
                            <div class="panel-heading">
                                <div class="widget-menu pull-left">
                                    ...
                                    <a href="add.php<?=$ob->gets_go($_GET, "module", $nameRazdel)?>" class="btn btn-system btn-sm">
                                        <span class="fa fa-plus"></span> Добавить языковую версию
                                    </a>
      ...
      //-->
                  ...
                  <td class="id_block_style" align="left">' . $res['name'] . '</td>
                  <td class="id_block_style" align="left">' . $res['name_reduction'] . '</td>
