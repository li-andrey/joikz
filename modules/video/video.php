...
class Video extends Module{
	...
	public $table = 'i_video';
	public $tableE = 'i_video_elements';

	public static $tableS = 'i_video';
	public static $tableES = 'i_video_elements';
	public static $link = 'video';
	public $url = 'video';
	...
	{
		$breadcrumb = new breadcrumb(new Request);
		return $breadcrumb->init(self::$tableS, self::$tableES, self::$link, $title);
	}

	public function init()
	{
		if ($this->aliasModule(str_replace('i_', '',$this->table), self::$link)=='module'){
			$this->listBlock();
		}else if ($this->aliasModule(str_replace('i_', '',$this->tableE), self::$link)){
			$this->listElement();
		}else{
			$this->page404(array());
		}
